...
// FIX: small helper to validate addresses before reads/writes
static inline bool is_valid_ptr(uintptr_t p) {
    return p && p > 0x1000;
}

uintptr_t FindMouseService(uintptr_t datamodel) {
    LOG_TRACE("FindMouseService: datamodel=0x%p", (void*)datamodel);
    uintptr_t mouse_service = utils::find_first_child_byclass(datamodel, "MouseService");
    LOG_TRACE("FindMouseService: result=0x%p", (void*)mouse_service);
    return mouse_service;
}

static Matrix3 LookAtToMatrix(const Vector& cameraPosition, const Vector& targetPosition) {
    // FIX: avoid division by zero when camera and target are nearly identical
    Vector delta = targetPosition - cameraPosition;
    float len = delta.Length();
    if (len < 0.01f) {
        return Matrix3();
    }

    Vector forward = delta / len;
    Vector right = Vector(0, 1, 0).Cross(forward);
...
uintptr_t GetCachedMouseService() {
    // FIX: removed static cached pointer to avoid stale pointer after game restart
    uintptr_t cached_mouse_service = 0;
    if (!is_valid_ptr(globals::datamodel))
        return 0;
    LOG_TRACE("GetCachedMouseService: fetching");
    cached_mouse_service = FindMouseService(globals::datamodel);
    if (!is_valid_ptr(cached_mouse_service))
        return 0;
...
std::uint64_t GetInputObject(std::uint64_t base_address) {
    ...
    // FIX: validate base_address before dereferencing
    if (!is_valid_ptr(base_address))
        return 0;

    std::uint64_t object_address = Read<std::uint64_t>(base_address + 0x118);
    if (!is_valid_ptr(object_address))
        return 0;
...
void WriteMousePosition(std::uint64_t address, float x, float y) {
    ...
    // FIX: validate input_object before writing
    if (!is_valid_ptr(input_object))
        return;
...
AimInstances GetAimInstances(uintptr_t player) {
    AimInstances aims{};
    LOG_TRACE("GetAimInstances: player=0x%p", (void*)player);
...
bool CAimbot::aim_at_closest_player(matrix view_matrix)
{
    ...
    LOG_TRACE("CAimbot::aim_at_closest_player: start");
    ...
    {
        LOG_TRACE("Attempting to lock players_mutex for aimbot");
        // FIX: avoid blocking render thread — try to acquire lock and skip frame if unavailable
        std::unique_lock<std::mutex> _g(globals::players_mutex, std::try_to_lock);
        if (!_g.owns_lock()) {
            LOG_TRACE("Skipping frame: couldn't lock players_mutex");
            return false; // skip current frame/iteration
        }
        LOG_TRACE("Mutex locked for aimbot - copying players_cached (size=%zu)", globals::players_cached.size());
    ...
    if (vars::aimbot::arsenal_tp_fix && locked_target)
    {
        ...
        if (character)
        {
            ...
            if (root)
            {
                ...
                if (primitive)
                {
                    // FIX: 'continue' is invalid here (not in loop), use return false or just skip
                    if (!is_valid_ptr(primitive)) return false;
        ...
        else
        {
            ...
            try
            {
                ...
                for (auto& player_instance : player_instances)
                {
                    ...
                    if (vars::aimbot::target_hitbox == 0) bone_name = "Head";
                    else if (vars::aimbot::target_hitbox == 1) bone_name = "HumanoidRootPart";
                    ...
                    if (!is_valid_ptr(primitive)) continue;
    ...
    } // ЗАКРЫВАЕМ IF (SHOULD_FIND_NEW_TARGET)
    ...
    if (vars::aimbot::prediction)
    {
        ...
        if (humanoid_root_part) {
            ...
            if (is_valid_ptr(h_r_p_primtive))
            {
    ...
    } // <--- ВОТ ЭТА СКОБКА БЫЛА ПРОПУЩЕНА (Закрывает prediction)
    ...
    return true;
} // <--- ЗАКРЫВАЕТ CAimbot::aim_at_closest_player
