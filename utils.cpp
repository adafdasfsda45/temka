...
namespace utils {
    ...
    uintptr_t find_first_child(uintptr_t instance_address, const std::string& child_name)
    {
        ...
        if (children.empty() || now - update_time > std::chrono::seconds(2))
        {
            ...
            try {
                ...
                int safety_counter = 0; // FIX: prevent infinite loops due to corrupted circular lists
                int iter_counter = 0;
                for (uintptr_t current = childArray; current < end; current += 16)
                {
                    if (++safety_counter > 1500) break;
                    auto child_instance = Read<uintptr_t>(current);
                    if (!child_instance)
                        continue;
                    if ((++iter_counter % 100) == 0) {
                        LOG_TRACE("FindChild loop iter: %d (instance=0x%p)", iter_counter, (void*)child_instance);
                    }
    ...
    std::vector<uintptr_t> children(uintptr_t instance_address)
    {
        ...
        {
            // Prevent pathological scans and give driver breathing room
            if (++safety_count % 10 == 0)
                std::this_thread::sleep_for(std::chrono::milliseconds(1));

            auto child_ptr = Read<uintptr_t>(instances);
            if (!child_ptr)
                continue;

            container.emplace_back(child_ptr);

            if (container.size() > 100)
                break; // limit scanning to reasonable number of children
        }
