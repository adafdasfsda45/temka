...
#include "entry.h"
#include <cmath>
// Extreme verbose logging support
#include <chrono>
#include <ctime>
#include <cstdarg>
#include <cstdio>
...
static inline const char* _get_filename_from_path(const char* path) {
...
static inline void log_trace_impl(const char* file_path, int line, const char* fmt, ...) {
    ...
    char timebuf[64];
    std::snprintf(timebuf, sizeof(timebuf), "%02d:%02d:%02d.%03d", local_tm.tm_hour, local_tm.tm_min, local_tm.tm_sec, (int)ms.count());

    const char* filename = _get_filename_from_path(file_path);

    std::printf("[%s] [%s:%d] ", timebuf, filename, line);
...
#define LOG_TRACE(fmt, ...) log_trace_impl(__FILE__, __LINE__, fmt, ##__VA_ARGS__)
