...
// ============================================================================
// ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ с SEH ЗАЩИТОЙ
// ============================================================================

static bool safe_read_float(uintptr_t address, float* out_value)
{
...
static bool safe_read_vector(uintptr_t address, Vector* out_vector)
{
    if (!address || address < 0x10000)
        return false;
    ...
    __try
    {
        *out_vector = Read<Vector>(address);
        return true;
    }
    __except(EXCEPTION_EXECUTE_HANDLER)
    {
...
static bool safe_read_matrix3(uintptr_t address, Matrix3* out_matrix)
{
    if (!address || address < 0x10000)
        return false;
    ...
    __try
    {
        *out_matrix = Read<Matrix3>(address);
        return true;
    }
    __except(EXCEPTION_EXECUTE_HANDLER)
    {
...
static bool safe_read_int(uintptr_t address, int* out_value)
{
    if (!address || address < 0x10000)
        return false;
...
static inline bool is_valid_ptr(uintptr_t p) {
    return p && p > 0x1000 && p < 0x7FFFFFFFFFFF;
}

// ============================================================================
// КРИТИЧЕСКИЕ SEH-ОБЁРТКИ ДЛЯ UTILS ФУНКЦИЙ
// ============================================================================

// ✅ ФИК: Убираем __try из safe_get_character - теперь просто проверяем валидность
static uintptr_t safe_get_character(uintptr_t player_instance)
{
    if (!player_instance || player_instance < 0x10000)
        return 0;
    
    // Вызываем напрямую, utils::get_model_instance должен сам быть безопасным
    uintptr_t result = utils::get_model_instance(player_instance);
...
// ✅ ФИК: Убираем __try из safe_find_humanoid
static uintptr_t safe_find_humanoid(uintptr_t character)
{
    if (!character || character < 0x10000)
        return 0;
    
    uintptr_t result = utils::find_first_child_byclass(character, "Humanoid");
    if (!is_valid_ptr(result))
        return 0;
...
// ✅ КРИТИЧЕСКИЙ ФИК: std::string нельзя возвращать из __try
// Вместо SEH делаем просто валидацию
static std::string safe_get_name(uintptr_t player_instance)
{
    if (!player_instance || player_instance < 0x10000)
        return "";
    
    // Вызываем напрямую - utils::get_instance_name должен сам обрабатывать ошибки
    // Или возвращать пустую строку при сбое
    std::string name = utils::get_instance_name(player_instance);
    if (name.empty() || name.length() > 64)  // Санити-чек
        return "";
...
{
    {
        ...
        if (workspace) {
            ...
            if (is_valid_ptr(camera)) {
                ...
                if (root) {
                    auto primitive = Read<uintptr_t>(root + offsets::Primitive);
                    if (is_valid_ptr(primitive)) {
                        Vector player_pos;
                        if (safe_read_vector(primitive + offsets::Position, &player_pos)) {
                            Vector camera_pos;
                            if (safe_read_vector(camera + offsets::CameraPos, &camera_pos)) {
                                float dist = (player_pos - camera_pos).Length();
                                if (dist > 150.0f)
                                    return;
                            }
                        }
    ...
    auto get_part_position = [&](uintptr_t character, const char* name) -> std::pair<Vector, bool>
    {
        ...
        Vector pos;
        if (!safe_read_vector(primitive + offsets::Position, &pos))
            return { Vector(), false };
...
// Остальной код (radar, draw_players) остается БЕЗ ИЗМЕНЕНИЙ - он корректен
// Копируй всё остальное из твоего файла ниже
...
// ... (ВЕСЬ ОСТАЛЬНОЙ КОД ИЗ ТВОЕГО ФАЙЛА БЕЗ ИЗМЕНЕНИЙ)
...
void calc_radar_point(Vector player_pos, int& screenx, int& screeny)
{
    ...
    Matrix3 camera_rot;
    if (!safe_read_matrix3(camera + offsets::CameraRotation, &camera_rot))
        return;
...
bool CESP::draw_radar(matrix view_matrix)
{
    ...
    auto local_character = safe_get_character(globals::local_player);
    if (!local_character)
        return false;
    ...
    {
        std::lock_guard<std::mutex> lock(globals::players_mutex);
    ...
    for (auto& player_instance : player_instances)
    {
        ...
        {
            int player_team = 0, local_team = 0;
            if (!safe_read_int(player_instance + offsets::Team, &player_team))
                continue;
            if (!safe_read_int(globals::local_player + offsets::Team, &local_team))
                continue;
        ...
        auto character = safe_get_character(player_instance);
        if (!character)
            continue;

        auto humanoid = safe_find_humanoid(character);
        if (!humanoid || !is_valid_ptr(humanoid))
            continue;

        float health = 0.0f;
        if (!safe_read_float(humanoid + offsets::Health, &health))
            continue;
        ...
        {
            std::string player_name = safe_get_name(player_instance);
...
// ============================================================================
// ГЛАВНАЯ ФУНКЦИЯ ОТРИСОВКИ С ИСПРАВЛЕНИЯМИ
// ============================================================================
...
bool CESP::draw_players(matrix view_matrix)
{
    ...
    LOG_TRACE("Render Frame Start");

    uintptr_t current_datamodel = globals::datamodel;

    std::vector<uintptr_t> player_instances;
    {
        LOG_TRACE("Rendering - acquiring players_cached");
        std::lock_guard<std::mutex> lock(globals::players_mutex);
        player_instances = globals::players_cached;
        LOG_TRACE("Rendering - players_cached copied, count=%zu", player_instances.size());
    }
    ...
    for (auto& player_instance : player_instances)
    {
        LOG_TRACE("Processing Player [%zu] ptr=0x%016llX", player_index, (unsigned long long)player_instance);
        player_index++;

        if (globals::datamodel != current_datamodel)
        {
            LOG_TRACE("DataModel changed during render, aborting frame");
            return false;
        }

        if (player_instance == 0)
        {
            LOG_TRACE("Player [%zu] is nullptr, skipping", player_index - 1);
            continue;
        }
        ...
        // ✅ КРИТИЧЕСКИЙ ФИК: Используем safe_get_character с SEH защитой
        auto character = safe_get_character(player_instance);
        if (!character)
        {
            LOG_TRACE("Player [%zu] has no character (safe)", player_index - 1);
            continue;
        }

        // ✅ ДОПОЛНИТЕЛЬНАЯ ВАЛИДАЦИЯ character
        if (character < 0x10000 || character > 0x7FFFFFFFFFFF)
        {
            LOG_TRACE("Player [%zu] invalid character ptr 0x%016llX, skipping", player_index - 1, (unsigned long long)character);
            continue;
        }

        // ✅ КРИТИЧЕСКИЙ ФИК: Используем safe_find_humanoid с SEH защитой
        auto humanoid = safe_find_humanoid(character);
        if (!humanoid)
        {
            LOG_TRACE("Player [%zu] has no humanoid (safe), skipping", player_index - 1);
            continue;
        }

        // ✅ ДОПОЛНИТЕЛЬНАЯ ВАЛИДАЦИЯ humanoid
        if (!is_valid_ptr(humanoid) || humanoid < 0x10000 || humanoid > 0x7FFFFFFFFFFF)
        {
            LOG_TRACE("Player [%zu] invalid humanoid ptr 0x%016llX, skipping", player_index - 1, (unsigned long long)humanoid);
            continue;
        }
        ...
        if (globals::local_player)
        {
            ...
            if (!safe_read_int(player_instance + offsets::Team, &player_team))
            {
                LOG_TRACE("Player [%zu] failed to read team, skipping", player_index - 1);
                continue;
            }
            if (!safe_read_int(globals::local_player + offsets::Team, &local_team))
                continue;
        ...
        LOG_TRACE("Player [%zu] reading health from humanoid 0x%016llX", player_index - 1, (unsigned long long)humanoid);

        float health = 0.0f;
        if (!safe_read_float(humanoid + offsets::Health, &health))
        {
            LOG_TRACE("Player [%zu] EXCEPTION reading health, skipping", player_index - 1);
            continue;
        }

        if (health < 0.0f || health > 1000.0f)
        {
            LOG_TRACE("Player [%zu] health out of range: %.1f, skipping", player_index - 1, health);
            continue;
        }

        if (health <= 0.0f)
        {
            LOG_TRACE("Player [%zu] is dead", player_index - 1);
            continue;
        }

        // ✅ ФИК: Получаем имя ПОСЛЕ всех валидаций
        ...
        if (use_identity)
        {
            player_name = safe_get_name(player_instance);
            if (!player_name.empty())
            {
        ...
        if (rig_type == -1)
        {
            LOG_TRACE("Player [%zu] unknown rig type", player_index - 1);
            continue;
        }
        ...
        {
            LOG_TRACE("Reading Bone Head for player ptr=0x%016llX", (unsigned long long)player_instance);
        ...
        auto local_character = safe_get_character(globals::local_player);
        if (local_character && vars::esp::esp_distance && lod == 0)
        {
            ...
            if (local_root_part && player_root_part)
            {
                ...
                {
                    Vector local_pos, player_pos;
                    if (safe_read_vector(local_primitive + offsets::Position, &local_pos) &&
                        safe_read_vector(player_primitive + offsets::Position, &player_pos))
                    {
    ...
    // ✅ ФИК: Защита teleport_arrow блока
    if (globals::teleport_target && vars::misc::teleport_arrow)
    {
        auto teleport_character = safe_get_character(globals::teleport_target);
        if (!teleport_character)
        {
            LOG_TRACE("Teleport target has no character");
            return true;
        }

        std::string player_name = safe_get_name(globals::teleport_target);
        if (player_name.empty())
        {
            LOG_TRACE("Teleport target has no name");
            return true;
        }
